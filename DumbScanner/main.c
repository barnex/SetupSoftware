...
int main(int argc, char **argv)
{
    ...
    sprintf(cfgfile, "%s/dumb_scanner.ini", homeDir );
    ...
    fprintf(dest, "# Scan parameters: power %f dBm\n", config.power);
    fprintf(dest, "# Fieldstrength [T]\tFrequency[MHz]\tMag. signal\n");
	...
	while( currentField <= fieldStop )
	{
	    ...
	    fprintf(dest, "%e\t%e\t%e\n", currentField, freqCurrent , rfpower );
	    fflush(dest);
