...
int gpib_lock_read(gpibio *self, int state) {
	return arduino_write_command( self->arduino, LOCKREAD, state ? BOOLEAN : 0 );
}
int gpib_remote(gpibio *self, int state)    {
	return arduino_write_command( self->arduino, REMOTE, state ? BOOLEAN : 0 );
}
int gpib_untalk(gpibio *self)		    {
	return arduino_write_command( self->arduino, UNTALK, 0 );
}
int gpib_unlisten(gpibio *self)		    {
	return arduino_write_command( self->arduino, UNLISTEN, 0 );
}
int gpib_lockout(gpibio *self)		    {
	return arduino_write_command( self->arduino, LOCKOUT, 0 );
}
int gpib_clear(gpibio *self, int bus)	    {
	return arduino_write_command( self->arduino, CLEAR, bus ? BOOLEAN : 0 );
}
int gpib_unlock(gpibio *self)		    {
	return arduino_write_command( self->arduino, UNLOCK, 0 );
}
int gpib_trigger(gpibio *self)		    {
	return arduino_write_command( self->arduino, TRIGGER, 0);
}
